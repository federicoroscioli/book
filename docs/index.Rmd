...
The authors also created also an ad-hoc **R playground** accessible
...
# Section 1 - The R code {.unnumbered}
...
![](images/8psue2xkvn811.jpg){width="600"}
...
2.  Run the installer keeping the default settings. If you do not have
    admin rights on your computer, please ask you IT Support to give you
    full permissions to the R directories. Otherwise you will not be
    able to install packages afterwards.
3.  Download the R-Studio installer from
    [R-Studio](https://rstudio.com/products/rstudio/download/#download).\
    ![Select the version of R-Studio according to your operating
    system.](/Volumes/New/lavori/corso%20R/materiali/rstudio.png)\
4.  Once the installation of R is completed (NOT BEFORE), run the
    R-Studio installer keeping the default settings.
5.  Run R-Studio. It should open an window like the one in the image
    below.\
    ![R-Studio.](/Volumes/New/lavori/corso%20R/materiali/rstudio2.png)\
6.  In the left hand window, by the sign "\>", type "4+5" (without the
    quotes) and hit enter. An output line reading "$$1$$ 9" should
    appear. This means that R and R-Studio are working properly. If this
    is not successful, please [contact
    me](mailto:%20federico.roscioli@gmail.com).
7.  Go to Tools -\> Install Packages and install the packages:"gt", and
    "readxl". See the image below.\
    ![R-Studio.](/Volumes/New/lavori/corso%20R/materiali/pkg1.png)\
8.  Check that the packages are installed by typing "library(gt)"
    (without the quotes) in the prompt and press enter.\
    ![R-Studio.](/Volumes/New/lavori/corso%20R/materiali/pkg2.png)\
9.  Finally type "sessioninfo()" (without the quotes) and check that gt
    has been installed.\
    ![R-Studio.](/Volumes/New/lavori/corso%20R/materiali/pkg3.png)
...
![The
Work-space.](/Volumes/New/lavori/corso%20R/materiali/workspace.png)
...
Download the [wine
dataset](http://www.federicoroscioli.com/wp-content/uploads/2021/02/Wine.csv),
...
with the [village
dataset](http://www.federicoroscioli.com/wp-content/uploads/2022/03/Village.xlsx),
...
# Section 2 - Statistical Analysis {.unnumbered}
...
![The six steps of data
analysis.](images/Schermata%202022-03-28%20alle%2010.36.15.png)
...
all values divided by the number of observations (formula 5.1).

$$
\begin{aligned}
\bar x = \frac{\sum{x}}{n}  && \text{(5.1)}
\end{aligned}
$$
...
$n$ values (formula 5.2).

$$
\begin{aligned}
\bar x_{g} = \sqrt[n^{th}]{x_1*x_2*...*x_n}  && \text{(5.2)}
\end{aligned}
...
(formula 5.3), regardless of the distribution of the data Thus it is
sensitive to outliers.

$$
\begin{aligned}
\frac{max-min}{2}  && \text{(5.3)}
\end{aligned}
$$
...
the maximum value minus the minimum value (formula 5.4). $$
\begin{aligned}
range=max-min  && \text{(5.4)}
\end{aligned}
$$
...
values in a distribution (formula 5.5).

$$
\begin{aligned}
IQR=75^{th} percentile - 25^{th} percentile  && \text{(5.5)}
\end{aligned}
$$
...
from the mean (formula 5.6).

$$
\begin{aligned}
\sigma^2=\frac{\sum{(X-\bar x)^2}}{N}  && \text{(5.6)}
\end{aligned}
$$
...
(formula 5.7). It is an especially useful measure of variability when
...
standard deviation to the mean (formula 5.8), and it is a useful
...
$$
\begin{aligned}
G(x)=1-\frac{2}{N-1}\sum^{N-1}_{i=1}{q_i}  && \text{(5.9)}
\end{aligned}
$$
...
index.](images/gini.png){width="180"}
...
$$
\begin{aligned}
Palma = \frac {top10}{bottom40}  && \text{(5.10)}
\end{aligned}
$$
...
specific range (i.e. $0, 1$). Min-Max scaling is important when we want
to retain the distance between the data points.

$$
\begin{aligned}
r_{ij}=\frac{x_{ij}-\min_ix_{ij}}{\max_ix_{ij}-\min_ix_{ij}} && \text{(5.11)}
\end{aligned}
$$
...
it can be described as a normal distribution. Normal distribution, or
...
$$
\begin{aligned}
z_{ij}=\frac{x_{ij}-\bar x_j}{\sigma_j} && \text{(5.12)}
\end{aligned}
$$
...
(generally the mean or maximum)(formula 5.13). This normalization allows
...
$$
\begin{aligned}
I_{ij}=\frac{x_{ij}}{x^*_{oj}}*100 && \text{(5.13)}
\end{aligned}
$$
...
by the sum of the values (formula 5.14). The sum of the normalized
values are equal to 100.

$$
\begin{aligned}
p_{ij}=\frac{x_{ij}}{\sum^n_{i=1}x_{ij}}*100 && \text{(5.14)}
\end{aligned}
$$
...
simple matter of division (formula 5.15).

$$
\begin{aligned}
p=\frac{n}{N}*100 && \text{(5.15)}
\end{aligned}
$$
...
```{r, echo=F, out.width=c('50%', '50%'), fig.show='hold', fig.cap="From left: A density distribution as outline of the hisogram; The probability that the random variable $X$ is higher than or equal to 1.5.", message=F, error=F}
library(ggplot2)
...
![The Empirical Rule.](images/Senza%20titolo.png)
...
```{r, echo=F, out.width=c('50%', '50%'), fig.show='hold', fig.cap="The comparison between weight and income standardized.", message=F, error=F}
library(ggplot2)
ggplot()+
        geom_density(aes(rnorm(10000)), alpha=.2, fill="#FF6666")+
...
```{r, echo=F, out.width=c('50%', '50%'), fig.show='hold', fig.cap="Single Sample T-Test as plotted by gginference package.", message=F, warning=F, error=F}
library(gginference)
ggttest(t.test(mtcars$mpg, mu = 22, alternative = "less"))
```

![](images/Schermata%202022-03-29%20alle%2016.14.37.png){width="350"}
...
![](images/Schermata%202022-03-29%20alle%2016.36.10.png){width="400"}
...
covary there are two possibilities. One is that the change in a thing is
concomitant with the change in another, as the change in a child's age
covaries with his height. The older, the taller. When higher magnitudes
on one thing occur along with higher magnitudes on another and the lower
magnitudes on both also co-occur, then the things vary together
positively, and we denote this situation as positive covariation or
**positive correlation**. The second possibility is that two things vary
inversely or oppositely. That is, the higher magnitudes of one thing go
along with the lower magnitudes of the other and vice versa. Then, we
denote this situation as negative covariation or **negative
correlation***.* This seems clear enough, but in order to be more
...
best known is the [Pearson product-moment correlation
coefficient]{.underline}, which is obtained by dividing the covariance
of the two variables ($\sigma_{xy}$) by the product of their standard
deviations.$$
\begin{aligned}
\rho=\frac{\sigma_{xy}}{\sigma_x*\sigma_y}  && \text{(5.4)}
\end{aligned}
$$Two variables $x$ and $y$ are positively correlated if when $x$
increases $y$ increases too and when $x$ decreases $y$ decreases too.
The correlation is instead negative when the variable $x$ increases and
the variable $y$ decreases and vice-versa. The sign of $\rho$ depends
only on the covariance ($\sigma_{xy}$). The correlation coefficient
varies between -1 (perfect negative linear correlation) and 1 (perfect
positive linear correlation), and if it is equal to zero the variables
are independent.
...
The last measure of relationship we will talk about is the **Cramér's
V** (sometimes referred to as Cramer's Phi). This is a measure of
association between two categorical variables (or categorical) based on
Pearson's chi-squared statistic and it was published by Harald Cramér in
1946. Cramér's V, gives us a method that can be used when we want to
study the intercorrelation between two discrete variables, and may be
used with variables having two or more levels. It varies from 0
...
determined by the other. Thus [it does not tell us the direction of the
...
the sum of squared error estimates for the given data set, thus
minimizing:

$$
\begin{aligned}
RSS=e_1^2+e_2^2+...+e_n^2 && \text{(1.2)}
\end{aligned}
$$
...
```{r, echo=F, warning=F, out.width=c('50%'), fig.show='hold', fig.cap="Plot of the residuals from a regression line."}
d <- cars
...
output.](images/Schermata%202022-07-07%20alle%2017.24.44.png){width="350"}
...
$$
\begin{aligned}
y\approx\beta_0+\beta_1x_1+\beta_2x_2...+\beta_px_p+e && \text{(1.3)}
\end{aligned}
$$
...
output.](images/Schermata%202022-07-07%20alle%2018.49.38.png){width="350"}
...
```{r fig2,  fig.show='hold', fig.cap="Dendogram plot."}
# Euclidean distance
...
```{r fig3, fig.show='hold', fig.cap="K-means clustering."}
...
```{r, echo=F, warning=F, out.width=c('50%', '50%'), fig.show='hold', fig.cap="Screeplot.", message=F}
library(factoextra)
#running the PCA
pca_swiss <- prcomp(swiss, scale = TRUE)
...
```{r,  fig.show='hold', fig.cap="From left: Pairs graph before PCA; Pairs graph after PCA.", message=F}
# Pairs before PCA
...
![Classification tree explanation. Source Lee
(2018).](images/classification-tree_ygvats%20copia.jpg){width="600"}
...
```{r, echo=F, fig.show='hold', fig.cap="The manually pruned CART.", message=F}
fancyRpartPlot(fit2, caption = "Classification Tree for Titanic pruned manually")
...
columns (individual indicators), we calculate the normalized matrix
$Z=z_{ij}$ as follows:
...
methodology with goalposts. Given the matrix $X=\{x_{ij}\}$ with $n$
rows (units) and $m$ columns (indicators), we calculate the normalized
matrix $R=\{r_{ij}\}$ as follow: $$
...
calculated with respect to \$200\$. In both cases, the range of
normalized values is $(70; 130)$. The normalized indicators can then be
aggregated. Denoting with $M_{r_i}$ and $S_{r_i}$, respectively, the
mean and standard deviation of the normalized values of the unit $i$,
the generalized form of AMPI is given by: $$
AMPI_i^{+/-}=M_{r_i} \pm S_{r_i}*cv_i
$$ where $cv_i=\frac{S_{r_i}}{M_{r_i}}$ is the coefficient of variation
of the unit $i$ and the sign $\pm$ depends on the kind of phenomenon to
be measured. If the composite index is increasing or positive, that is,
